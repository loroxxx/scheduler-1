...
package com.jinhui.scheduler.biz.cmbfae.writer;
...
/**
 * 反编译自FlatFileItemWriter类，
 * 只重写了write()方法，把传进来的对象转为json字符串
 */
public class JsonFileItemWriter<T> extends AbstractItemStreamItemWriter<T> implements ResourceAwareItemWriterItemStream<T>,
        InitializingBean {

    private static final boolean DEFAULT_TRANSACTIONAL = true;

    protected static final Log logger = LogFactory.getLog(JsonFileItemWriter.class);
...
//		Assert.notNull(lineAggregator, "A LineAggregator must be provided.");
    ...
    public void write(List<? extends T> items) throws Exception {
        ...
        for (T item : items) {
            ...
            //把item转为json格式
            String dString = JSON.toJSONString(item, SerializerFeature.WriteMapNullValue);
            lines.append(dString + lineSeparator);
            lineCount++;
        }
        try {
